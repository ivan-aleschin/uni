...
namespace Lab6.Services
{
    public class MatrixService
    {
        ...
        public int[,] CreateRandomAdjacencyMatrix(int size, double density = 0.3)
        {
            ...
            for (int i = 0; i < size; i++)
            {
                for (int j = 0; j < size; j++)
                {
                    ...
                    else if (rnd.NextDouble() < density)
                    {
                        matrix[i, j] = 1;
                    }
                    else
        ...
        public int[,] ConvertToWeightedMatrix(int[,] adjacencyMatrix, int infinity)
        {
            int n = adjacencyMatrix.GetLength(0);
            var weightedMatrix = new int[n, n];
            Random rnd = new Random();
            ...
            for (int i = 0; i < n; i++)
            {
                for (int j = 0; j < n; j++)
                {
                    ...
                    {
                        // Случайный вес от 1 до 9
                        weightedMatrix[i, j] = rnd.Next(1, 10);
                    }
                    else
                    {
