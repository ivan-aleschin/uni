...
sequenceDiagram
    ...
    Director->>Builder: setDriver("Михаил Кузнецов", "B")
    ...
    Builder-->>Client: unique_ptr<Vehicle>
    deactivate Builder
```
...
- **Строитель (Builder)** делает акцент на *пошаговом* конструировании сложного объекта. Он позволяет создавать разные представления одного и того же объекта (например, такси с детским креслом или без). Продукт возвращается клиенту только на самом последнем шаге.
- **Абстрактная фабрика (Abstract Factory)** делает акцент на создании *семейств* связанных или зависимых объектов (например, кресло, стол и диван в одном стиле). Объекты возвращаются клиенту немедленно.

**2. Достоинства и недостатки паттернов проектирования «Абстрактная фабрика» и «Строитель».**

*Абстрактная фабрика:*
- **Достоинства:** Гарантирует сочетаемость создаваемых объектов (они точно из одного семейства); изолирует клиентский код от конкретных классов.
- **Недостатки:** Сложно добавлять новые типы продуктов в семейство, так как для этого потребуется менять интерфейс самой фабрики и всех ее конкретных реализаций.

*Строитель:*
- **Достоинства:** Позволяет изменять внутреннее представление конструируемого продукта; изолирует сложный код сборки от клиентского кода; дает более тонкий контроль над процессом конструирования.
- **Недостатки:** Конкретный Строитель жестко привязан к создаваемому им классу продукта, поэтому при изменении продукта придется менять и класс соответствующего Строителя. Паттерн может неоправданно усложнить код, если структура объектов не очень сложная.