...
| `include/DeansOffice.h` + `src/DeansOffice.cpp` | `DeansOffice` *(наследует от `IObserver`)* | конкретный класс | **ConcreteObserver** | Деканат. В `watch(teacher)` подписывается на преподавателя, в `update()` запоминает факт сдачи, в `checkWeek()` сверяет полный список и оповещает кафедру о пропустивших. |
...
| `src/main.cpp` | `main()` | функция | **Client** | Создаёт преподавателей, деканат и кафедру; деканат подписывается на каждого преподавателя; запускает сценарий из трёх недель. |

**Короткая защитная формулировка:** «`IObservable` и `IObserver` — два абстрактных класса-интерфейса (Subject и Observer из методички). Конкретный субъект — `Teacher`, конкретный наблюдатель — `DeansOffice`. Преподаватель в момент `submitGrades()` уведомляет всех подписавшихся через `notifyObservers()`. Деканат подписывается на нескольких преподавателей, в конце недели сверяет, кто прислал уведомление, и о пропустивших отдельно оповещает кафедру (`Department`) — это уже не часть паттерна, а бизнес-логика наблюдателя. `SubmissionInfo` — структура-DTO, аналог `StockInfo` из примера биржи в методичке.»
